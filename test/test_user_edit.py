...
class TestUserEdit(BaseCase):
    ...
    def test_edit_user_not_auth(self):
        # GET
        response1 = MyRequests.get(f"/user/2")
        before_username = self.get_json_value(response1, "username")

        new_name = "Changed Name"
        response = MyRequests.put(f"/user/2", data={"firstName": new_name})
    ...
    def test_edit_user_other_auth(self):
        ...
        response1 = MyRequests.post("/user/", data=user_1_register_data)
        Assertions.assert_code_status(response1, 200)
        Assertions.assert_json_has_key(response1, "id")
        ...
        response2 = MyRequests.post("/user/", data=user_2_register_data)
        ...
        # Edit
        new_name = "Changed Name"
        response5 = MyRequests.put(f"/user/{user_id_2}",
                                 headers={"x-csrf-token": token_1},
                                 cookies={"auth_sid": auth_sid_1},
                                 data={"firstName": new_name}
                                 )
